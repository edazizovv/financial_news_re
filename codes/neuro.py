...
class Gene(nn.Module):
    ...
    def forward(self, x_categorical, x_numerical):
        if self.data.data.any_categorical:
            embeddings = []
        ...
        if self.data.data.any_categorical and self.data.data.any_numerical:
        ...
        elif not self.data.data.any_categorical and self.data.data.any_numerical:
            x = torch.cat([x_numerical], 1)
        # if self.data.data.categorical_embeddings is not None and self.data.data.numerical_shape is None:
        elif self.data.data.any_categorical and not self.data.data.any_numerical:
